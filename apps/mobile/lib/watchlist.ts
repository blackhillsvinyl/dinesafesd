/**
 * Local watchlist storage, persisted with AsyncStorage.
 *
 * Watchlist = restaurants a user wants to follow (separate from favorites/hearts).
 */

import AsyncStorage from '@react-native-async-storage/async-storage';

const STORAGE_KEY = 'dinesafesd_watchlist';
...
export async function toggleWatchlist(id: string): Promise<boolean> {
  const s = await load();
  const nowWatching = !s.has(id);
  if (nowWatching) s.add(id);
  else s.delete(id);
  await save(s);
  return nowWatching;
}
